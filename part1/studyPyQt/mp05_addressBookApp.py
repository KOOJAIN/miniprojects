...
class qtApp(QMainWindow):
    ...
    def __init__(self) :
        ...
        self.setWindowIcon(QIcon('./studyPyQt/addressbook.ui.png'))
        ...
        self.btnDel.clinced.connect(self.btnDelClicked)

    def btnDelClicked(self):
        pass


    def btnNewClicked(self): # 신규버튼 누를때 발생하는 함수
    ...
    def makeTable(self, rows):
        ...
        for i, row in enumerate(rows):
            #print(row[4]) # row[0] ~ row[4] 사용할 수있다
            idx = row[0]
            fullName = row[1]
            phoneNum = row[2]
            email = row[3]
